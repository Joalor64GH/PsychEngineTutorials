# NOTE: This tutorial requires Artemis and a special plugin!
* https://artemis-rgb.com/
* https://github.com/skedgyedgy/Artemis.Plugins.FNF
# Artemis Integration

The files that will be modified/added:

* Project.xml (M)
* fnf-vanilla.json (A)
* pack.json (M)
* AchievementsMenuState.hx (M)
* ArtemisIntegration.hx (A)
* ClientPrefs.hx (M)
* CreditsState.hx (M)
* FreeplayState.hx (M)
* FunkinLua.hx (M)
...
* TitleState.hx (M)
* ChartingState.hx (M)
* EditorPlayState.hx (M)
* MasterEditorMenu.hx (M)
* WeekEditorState.hx (M)
* BaseOptionsMenu.hx (M)
* ControlsSubState.hx (M)
* GraphicsSettingsSubState.hx (M)
* NotesSubState.hx (M)
* OptionsState.hx (M)
* VisualsUISubState.hx (M)

## Step 1: Editing/Adding a few files

In `Project.xml`, after:
```xml
...
Now, make a new folder in `assets`, and name it, `artemis`.

Then in that folder, make a file called, `fnf-vanilla.json`.

After that, copy and paste the text from this link into the file:

https://raw.githubusercontent.com/Joalor64GH/Joalor64-Engine/main/assets/artemis/fnf-vanilla.json

## Step 2: Editing the Source
### 2.1: Editing `AchievementsMenuState.hx`
...
### 2.2: Adding `ArtemisIntegration.hx`

Make a new file called `ArtemisIntegration.hx`.
