...
## 1. Creating `ToastCore.hx`
First of all, make a folder in the source called `core`.

Then, make a file called `ToastCore.hx`.

After that, copy and paste the following text:
...
# 2. Editing `Main.hx`
For `Main.hx`, add this in the import section:
```hx
...
Then, in `public function new()`, add this:
```hx
toast = new ToastCore();
addChild(toast);
```
